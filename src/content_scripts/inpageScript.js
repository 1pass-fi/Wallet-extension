...
const inpageScript = () => {
  ...
  window.arweaveWallet = {
    ...
    signature: (hashBuffer, algorithm) => buildPromise(MESSAGE_TYPES.SIGNATURE, { hashBuffer, algorithm })
  }
...
const inpageScriptWithoutArweaveWallet = () => {
  const promiseResolves = {}
  Object.values(MESSAGE_TYPES).forEach(messageType => {
    promiseResolves[`${messageType}_SUCCESS`] = []
    promiseResolves[`${messageType}_ERROR`] = []
...
export default (disabledArweave) => {
  if (disabledArweave) return inpageScriptWithoutArweaveWallet
  else return inpageScript
}
