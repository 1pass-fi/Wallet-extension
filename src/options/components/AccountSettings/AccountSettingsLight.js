...
const AccountSettingsLight = React.forwardRef(({ className, toggleDropdownMenu }, ref) => {
  return (
    <div
      ref={ref}
      className={clsx(
        'z-50 flex flex-col w-58.5 3xl:w-68.5 bg-trueGray-200 text-blue-600 px-2 justify-evenly font-semibold text-sm 3xl:text-base rounded shadow-md',
        className
      )}
    >
      {accountSettingItems.map(({ text, path, disabled, icon: Icon }, idx) =>
        disabled ? (
          <div
            className={clsx(
              'h-12 3xl:h-14 pl-2 text-trueGray-500 cursor-default flex items-center',
              idx !== 0 && 'border-t-2 border-trueGray-150 '
            )}
          >
            <Icon className="w-6 3xl:w-7 mr-4 3xl:mr-6" />
            {text}
          </div>
        ) : (
          <NavLink
            ...
            className={clsx(
              'h-12 3xl:h-14 pl-2 flex items-center hover:underline underline-offset-1',
              idx !== 0 && 'border-t-2 border-trueGray-150'
            )}
